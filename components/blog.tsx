...
export default function Blog() {
  ...
  return (
    <section className="relative w-full py-28 px-4 sm:px-6 lg:px-8 bg-white dark:bg-slate-950 overflow-hidden transition-colors duration-300">
      {/* Subtle Ambient Background Gradients for depth */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        >
          <div className="max-w-3xl text-left">
            <span className="text-emerald-600 dark:text-emerald-400 font-semibold text-s  tracking-wider mb-2 block">
              Our Blog & Insights
            </span>
            <h2 className="text-3xl sm:text-4xl font-bold text-slate-800 dark:text-white tracking-tight">
              Latest Articles & News
            </h2>
            <p className="mt-5 pb-2  text-base sm:text-lg font-normal text-slate-600 dark:text-slate-400 tracking-wide">
          ...
          <Link
            href="/blog"
            className="hidden sm:inline-flex items-center justify-center gap-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 px-6 py-3.5 text-sm font-semibold text-slate-800 dark:text-slate-200 transition-all duration-300 group shrink-0"
          >
            <span>Browse All Articles</span>
            <ArrowRight className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1 text-emerald-600 dark:text-emerald-400" />
          </Link>
        </div>

        {/* Editorial-Grade Card Grid */}
              ...
              <div className="flex-1 flex flex-col justify-between p-6 sm:p-7 space-y-6">
                <div className="space-y-3">
                  ...
                  <Link href={`/blog/${post.slug}`} className="block">
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors duration-200 line-clamp-2">
                      {post.title}
                    </h3>
                  </Link>
